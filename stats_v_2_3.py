...
def t_table(df, prop=0.05, tail=2, n=10000):
	"""
...
def p_of_t(t, df, tail=2, tx = 0.05, n=10000):
	"""
	...
	res = round(abs(dx * (Efxk + outers))* tail, 3)
	
	if res <= tx:
		return f"Null hypothysis rejected! p={res}"
	else:
		return f"Null hypothysis accepted. p={res}"


def t_indep(lst1, lst2):
	""" t value for the t test for independent means"""
