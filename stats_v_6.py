...
def p_of_t(t, df, tail=2, p = 0.05, n=10000):
	...
	if res <= p:
		print( f"Null hypothysis rejected! \nat p<{p}%, and {tail}-tailed probability distribution, as p={res}")

	else:
		print(f"Null hypothysis accepted. p\nat p<{p}%, and {tail}-tailed probability distribution, as p={res}")
...
def f_p(d1, d2, p=1.0, a=1000, n = 10**6):
...
def reject_null_h_f(f_v, d1, d2, p=0.05, a=1000, n=10**6 ):
	"""
	Null hypothesis rejected or not
